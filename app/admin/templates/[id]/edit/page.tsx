...
export default function EditTemplatePage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTemplate({
          ...
          preheader: '', // TODO: Add preheader support
          status: data.status
        })
      } else {
  ...
  const handleSave = async (isDraft = true) => {
    ...
    if (!template.name || !template.subject || !template.category) {
      toast.error('Por favor completa los campos requeridos: Nombre, Asunto y Categoría')
      return
    }

    if (!template.content) {
      toast.error('Por favor agrega contenido a la plantilla')
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTemplate(prev => ({ ...prev, status: updatedTemplate.status }))
        toast.success(`Plantilla ${isDraft ? 'guardada como borrador' : 'activada'} correctamente`)
        // No redirigir, mantener en la página de edición
      } else {
        const error = await response.json()
        toast.error(`Error al guardar plantilla: ${error.error || 'Error desconocido'}`)
      }
    } catch (error) {
      console.error('Error saving template:', error)
      toast.error('Error al guardar plantilla. Por favor intenta de nuevo.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        toast.error(`Error al eliminar plantilla: ${error.error || 'Error desconocido'}`)
      }
    } catch (error) {
            ...
            <TabsContent value="basic" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Modifica los datos básicos de tu plantilla de email
                  </CardDescription>
                </CardHeader>
            ...
            <TabsContent value="content" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Modifica el contenido HTML de tu plantilla. Usa variables para personalizar el contenido.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
          ...
          <Card>
            ...
            <CardContent>
              <ul className="space-y-2 text-sm text-muted-foreground">
                ...
                <li>• Guarda como borrador para probar cambios</li>
              </ul>
            </CardContent>
          </Card>
          ...
          <Card className="border-red-200">
            ...
            <CardContent>
              <p className="text-sm text-red-600 mb-4">
                Eliminar esta plantilla la borrará permanentemente y no se podrá recuperar.
              </p>
              <Button 
                variant="outline"
                onClick={handleDelete}
